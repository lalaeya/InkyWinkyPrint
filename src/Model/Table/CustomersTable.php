...
class CustomersTable extends Table
{
    ...
    public function initialize(array $config): void
    {
        ...
        $this->setDisplayField('cus_fname');
    ...
    public function validationDefault(Validator $validator): Validator
    {
        $validator
            ->scalar('cus_fname')
            ->maxLength('cus_fname', 100)
            ->requirePresence('cus_fname', 'create')
            ->notEmptyString('cus_fname');

        $validator
            ->scalar('cus_email')
            ->maxLength('cus_email', 100)
            ->requirePresence('cus_email', 'create')
            ->notEmptyString('cus_email');

        $validator
            ->scalar('cus_phonenumber')
            ->maxLength('cus_phonenumber', 15)
            ->requirePresence('cus_phonenumber', 'create')
            ->notEmptyString('cus_phonenumber');
